...
fifaAll <- read.csv("data2.csv", header = T, stringsAsFactors = F, encoding = "CP949")
fifa <- fifaAll[c(1:900),]
str(fifa)
...
ggplot(preferFoot,aes("",preferFoot$`선호하는 선수의 수`,fill = preferFoot$`선호하는 발`)) + geom_bar(stat = "identity") + coord_polar("y", start=0) + 
  ...
  theme_classic() + theme(axis.line = element_blank(),
          ...
          plot.title = element_text(hjust = 0.5, color = "#666666", size = 22)) +
...
boxplot(fifa$Height, main = "키 분포 상자그림", ylab = "키(cm)")
...
boxplot(fifa$Weight,  main = "몸무게 분포 상자그림", ylab = "몸무게(kg)")
...
#전체 선수들의 나이 분포 
kable(table(fifaAll$Age))

#나이에서 10,20,30,40대를 설정해주는 함수 
whatAge <- function(x){
  a <- c()
  for(i in 1 : nrow(fifaAll)){
    if(x[i] < 20){
      a[i] = 10
    }else if(x[i] < 30){
      a[i] = 20
...
tenTofourty <- table(whatAge(fifaAll$Age))
tenTofourty

AgeCount <- data.frame(tenTofourty)
agePct <- round(tenTofourty/sum(tenTofourty) * 100,2)
agePct <- paste0(names(tenTofourty),"대\n",tenTofourty,"명\n",agePct,"%")

#선수들의 분포 

ggplot(AgeCount,aes("",Freq,fill = Var1)) + geom_bar(stat = "identity") + coord_polar("y", start=90) + 
  geom_text(aes(label =c(agePct[1:3],"")), position = position_stack(vjust = 0.5), cex = 4) +
  theme_classic() + theme(axis.line = element_blank(),
          axis.text = element_blank(),
          axis.ticks = element_blank(),
          plot.title = element_text(hjust = 0.5, color = "#666666", size = 35)) +
  labs(x = NULL, y = NULL, fill = "나이", title = "선수들의 나이분포")
...
ggplot(richClub10,aes("",richClub10$`소속 선수 수(명)`,fill = factor(richClub10$Club))) + geom_bar(stat = "identity") + coord_polar("y", start=90) + 
  ...
  theme_classic() + theme(axis.line = element_blank(),
          axis.text = element_blank(),
          axis.ticks = element_blank(),
          plot.title = element_text(hjust = 0.5, color = "#666666",size = 15)) + labs(x ="" ,y = "", fill = "소속 선수 수(명)", title = "선수가치가 높은 상위 10개의 구단")
